...
`nmap -F <target>` To quickly discover common open ports.
...
By using `ping`. It show which TTL the target use
...
255 Mean the target use Cisco/BSD 

## Challenge 5 : Anonymous LDAP Query 
`ldapsearch -x -H ldap://<IP> -b "dc=example,dc=com"` To see if the target has LDAP service running or not.
...
If not it shows like this.

## Challenge 6 : Anonymous FTP Login 
`ftp <ip>` To login into Target's FTP but if the target doesn't have FTP it will shows like this :
...
`nmap -O <IP>` Show Version and The Accuarcy of possibility of OS's target
